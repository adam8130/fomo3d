...
export function Round({ endTime }) {
  ...
  const { contract, jackpot, roundId, userOwnedKeys, isColddowning, setInfoDialog } = useStore()
  ...
  useEffect(() => {
    ...
    const getVaultInfo = async () => {
      ...
      const earning = toNumber(vaultInfo.avaliable) / 1000000
      setUserEaring(earning)
    }
    getVaultInfo()
  }, [contract])
  ...
  return (
    <div className="py-3 p-5 flex flex-col gap-[18px] text-white relative">
      ...
      <h1 className="text-[32px] text-center">Contract will {!isColddowning ? 'drain' : 'start'} in</h1>
      {/* <div 
      ...
      <img className="absolute right-5 top-5 w-[20px] h-[20px]" src={Question_Icon} alt="information" onClick={() => setInfoDialog(RoundInfo)} />
    </div>
  );
}
